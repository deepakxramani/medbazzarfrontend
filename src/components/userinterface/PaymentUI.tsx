...
export default function PaymentUI(props: PaymentUIProps) {
  ...
  const razorpayAPIKey = import.meta.env.VITE_SERVER_URL_RAZORPAY_API_KEY;

  const productFromRedux = props.products;
  var product: Product[] = Object.values(productFromRedux);

  var totalamount = product.reduce((p1: number, p2: Product) => {
    var amt = Number(p2.qty) * Number(p2.price);
    return p1 + amt;
  }, 0);

  var amount = product.reduce((p1: number, p2: Product) => {
    var amt =
      Number(p2.qty) *
      (Number(p2.offerprice) !== 0 ? Number(p2.offerprice) : Number(p2.price));
    return p1 + amt;
  }, 0);
  ...
  const generateOrder = async (paymentid) => {
    var result = await postData('users/save_order', {
      ...
      orderdate: props?.userData?.orderdate,
  ...
  return (
    ...
    >
      <Grid container>
        <Grid item xs={12}>
          <p style={{ margin: 5, fontSize: 17, fontWeight: 500 }}>
            Payment Details
          </p>
        </Grid>
      ...
      >
        ...
        >
          <InfoOutlinedIcon />
          <p style={{ fontSize: 13 }}>
            Shop for ₹499 or more to get free delivery
          </p>
        </Grid>
        <Grid item xs={12}>
