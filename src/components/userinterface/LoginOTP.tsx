...
export default function LogInOTP() {
  ...
  const generateOTP = () => {
    const myotp = String(Math.floor(Math.random() * 8999) + 1000);
    alert(myotp);
    setOtp(myotp);
  };
  ...
  return (
    ...
    >
      {status ? (
        <Paper
          elevation={5}
          style={{ width: '90%', borderRadius: '60px 10px' }}
        >
          <Grid
            container
            spacing={2}
          ...
          >
            <Grid item xs={11} sm={8} md={6}>
              <Grid item xs={12}>
                <div
                  style={{ fontWeight: 'bold', fontSize: 30, marginBottom: 5 }}
                >
              ...
              <Grid item xs={12} style={{ marginTop: 80 }}>
                <Button onClick={handleOTP} variant='contained' fullWidth>
                  Get OTP
                </Button>
              </Grid>
        ...
        <GetOTP userData={userData} mobileno={mobileno} otp={otp} />
      ) : (
        <LogInDetails mobileno={mobileno} otp={otp} />
      )}
    </div>
  );
}
