import { AppBar, Box, Toolbar, Paper, InputBase, IconButton,Badge,Tooltip,Zoom } from "@mui/material";
...
export default function Header(props)
{
  ...
  const secondrySearchBar=()=>{
   return( 
   <Box sx={{flexGrow: 1,position:'relative',marginTop:props.marginTop}}>
    <AppBar style={{height: 80,background: '#fff', justifyContent: 'center',marginTop:matchesSM?0:70}} position={props.position}>
      <Toolbar style={{display: 'flex', justifyContent: 'space-between'}}>
      <MenuOutlinedIcon onClick={handleDrawer} style={{color: 'black', fontSize: 33}}/>
        {searchBarComponent()}
        <div style={{display: 'flex',justifyContent: 'space-between' ,width:70}}>
        <div style={{display:'flex',justifyContent:'center',alignItems:'center',flexDirection:'column'}}>
            <PersonOutlineOutlinedIcon onClick={()=>navigate('/loginscreen')} style={{color: 'black', fontSize: 33}}/>
  ...
  const searchBarComponent=()=>{
      return (
        <Paper
          component="form"
          sx={{ p: '2px 4px', display: 'flex', alignItems: 'center', width: '60%', boxShadow: 2, borderRadius: 50, margin:1 }}
        >
      ...
      <AppBar style={{height: 80,background: '#fff', justifyContent: 'center'}} position={props.position}>
        <Toolbar style={{display: 'flex', justifyContent: 'space-between'}}>
          ...
          <div style={{display: 'flex',justifyContent:'space-between',width:!matches?110:50,cursor:'pointer' }}>
              {!matches?
              <Tooltip title={userData?'':'Sign In'} TransitionComponent={Zoom} placement="bottom">
                <div style={{display:'flex',justifyContent:'center',alignItems:'center',flexDirection:'column',marginRight:15}}>
                  <PersonOutlineOutlinedIcon  onClick={()=>navigate("/loginscreen")} style={{fontSize:32,color:'#000'}}/> 
                  <div style={{fontSize:'0.9rem', fontWeight:'bolder',color:'#000'}}>{userData}</div>
           </div></Tooltip>:<div></div>}
              <Badge style={{marginRight:15}} badgeContent={keys?.length} color="primary">

                <div onClick={()=>handleCartVisit()} onMouseOver={showCartDetails} >
                  <ShoppingCartOutlinedIcon   style={{color: 'black', fontSize: 32}}/>
                </div>
              </Badge>
              {!matches?<PhoneOutlinedIcon  style={{color: 'black', fontSize: 32,marginRight:15}}/>:<div></div>}
          </div>
        </Toolbar>
      </AppBar>
      <div>
