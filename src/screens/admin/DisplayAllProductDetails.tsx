...
export default function DisplayAllProductDetails() {
  ...
  const fetchAllBrands = async (scid) => {
    var result = await postData('brands/fetch_all_brands_by_subcategoryid', {
      subcategoryid: scid,
    });
    if (result && result.status) {
  ...
  const fetchAllProducts = async (bid: any) => {
    var result = await postData('products/fetch_all_products_by_brandid', {
      brandid: bid,
    });
    if (result && result.status) {
  ...
  const handleError = (label: any, msg: any) => {
  ...
  const handleEditData = async () => {
    ...
    var result = await postData(
      'productdetails/edit_product_details_data',
      body,
    );
    if (result.status) {
  ...
  const handleEditPicture = async () => {
    ...
    if (Array.isArray(picture.bytes)) {
      picture.bytes.forEach((file, i) => {
    ...
    var result = await postData(
      'productdetails/edit_product_picture',
      formData,
    );
    if (result.status) {
      toast.success(result.message);
  ...
  const showImages = () => {
    // Single image string (server URL or blob URL from single file pick)
    if (typeof picture?.file === 'string' && picture.file) {
      return (
        <Avatar
          src={picture.file}
          variant='rounded'
    ...
    // Array of File objects (multi-file upload)
    if (Array.isArray(picture?.file)) {
        ...
        <DialogContent>
          <div style={useStyles.boxDisplayPD}>
            <Grid container spacing={3}>
              <Grid item xs={12}>
                <TitleComponent
                  title='Insert Product Details'
                  logo='logo.png'
                  listicon='list.png'
                  page='/displayallproductdeatils'
                />
              </Grid>
              ...
              <Grid item xs={3}>
                <TextField
                  value={quantity}
                  onFocus={() => handleError('quantity', null)}
              ...
              <Grid item xs={3}>
                <FormControl fullWidth>
                  ...
                  >
                    <MenuItem value='Deal of the day!'>
                      Deal of the day!
                    </MenuItem>
  ...
  return (
    <div style={useStyles.root}>
      <div style={useStyles.boxDisplay}>{ShowAllProductDetails()}</div>
