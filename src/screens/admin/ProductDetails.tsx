...
export default function ProductDetails() {
  ...
  const [error, setError] = useState<ProductDetailsError>({});
  ...
  const fillAllConcerns = () => {
    return concernList.map((item) => {
      return (
        <MenuItem key={item.concernid} value={item.concernid}>
          {item.concernname}
        </MenuItem>
      );
    });
  ...
  const handleSubmit = async () => {
    ...
    if (submit) {
      ...
      var result = await postData(
        'productdetails/submit_product_details',
        formData,
      );
        ...
        <Grid container spacing={3}>
          <Grid item xs={12}>
            <TitleComponent
              ...
              logo='logo.png'
              listicon='list.png'
              page='/admindashboard/displayallproductdetails'
            />
          </Grid>
          ...
          <Grid item xs={6}>
            ...
            {error.picture ? (
              <span
                style={{ color: '#d32f2f', fontSize: 12, marginLeft: 12.5 }}
              >
                {error.picture}
              </span>
            ) : (
